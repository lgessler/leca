...
class BeamSearch(Search):
    ...
    def step(self, step, lprobs, scores, tgt_dict=None):
        ...
        torch.topk(
            lprobs.contiguous().view(bsz, -1),
            k=min(
                # Take the best 2 x beam_size predictions. We'll choose the first
                # beam_size of these which don't predict eos to continue with.
                beam_size * 2,
                lprobs.contiguous().view(bsz, -1).size(1) - 1,  # -1 so we never select pad
            ),
...
class LengthConstrainedBeamSearch(Search):
    ...
    def step(self, step, lprobs, scores, tgt_dict=None):
        ...
        return self.beam.step(step, lprobs, scores, tgt_dict=tgt_dict)
